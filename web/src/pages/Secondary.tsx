...
const tutorialSteps = [
  { icon: "◐", title: "Dispute o Eclipse", body: "Cada carta pode mover o medidor entre Aurora (−3) e Noite (+3). Atingir um extremo desperta efeitos totais até o fim da rodada." },
  { icon: "♙", title: "Escolha a Postura", body: "Predação pressiona, Vigília protege e Arcano prepara Ritos. Os dois jogadores escolhem em segredo e revelam juntos." },
  { icon: "✦", title: "Encadeie Sigilos", body: "Cartas emitem Presa, Sol, Espelho, Garra, Cinza ou Coroa. A ordem da sua Trilha de Ressonância abre bônus únicos." },
  { icon: "⬡", title: "Responda com Guarda", body: "Quando um Assalto é anunciado, o defensor recebe uma janela clara: jogar no máximo uma Guarda ou passar." },
  { icon: "◆", title: "Gerencie Essência", body: "Essência paga cartas e cresce ao longo das rodadas. A temporária é gasta primeiro e expira no Crepúsculo." },
  { icon: "⚔", title: "Vença pela decisão", body: "Leve a Vitalidade rival a zero. Todas as contas Alpha têm as mesmas ferramentas competitivas." },
];

export function TutorialPage() {
  const [step, setStep] = useState(0);
  const current = tutorialSteps[step];
  return <div className="page tutorial-page"><header><p className="eyebrow">RITO DE INICIAÇÃO · {step + 1}/{tutorialSteps.length}</p><h1>Fundamentos do Véu</h1><div className="tutorial-progress">{tutorialSteps.map((_, index) => <button type="button" aria-label={`Ir à etapa ${index + 1}`} className={index <= step ? "is-complete" : ""} onClick={() => setStep(index)} key={index} />)}</div></header><section className="tutorial-card"><div className="tutorial-illustration" aria-hidden="true"><span>{current.icon}</span></div><div><p className="eyebrow">FUNDAMENTO {step + 1}</p><h2>{current.title}</h2><p>{current.body}</p><div className="tutorial-tip"><strong>Na mesa</strong><span>{step === 0 ? "O medidor permanece sempre no centro e anuncia quando um estado total foi ativado." : step === 3 ? "A faixa de Guarda pulsa e o cronômetro permanece visível." : "Passe o foco pelas cartas para ler o texto completo antes de agir."}</span></div></div></section><footer><button className="ghost-button" type="button" onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0}>← Voltar</button>{step < tutorialSteps.length - 1 ? <button className="primary-button" type="button" onClick={() => setStep(step + 1)}>Continuar →</button> : <Link className="primary-button" to="/decks">Montar meu deck</Link>}</footer></div>;
}
