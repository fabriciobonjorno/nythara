...
const tutorialSteps = [
  ...
  { phase: "Aparência", navLabel: "Avatar", icon: "champion", title: "Avatar é somente visual", body: "Os dez personagens representam você na Arena, mas não concedem Passiva, Ultimate, Vitalidade extra ou restrição de facção. A disputa é decidida pelas 30 cartas.", tip: "Escolha por identidade. Nenhum Avatar é mais forte que outro." },
  { phase: "Preparar", navLabel: "30 cartas", icon: "deck", title: "Monte um baralho legal", body: "Use exatamente 30 cartas, com no mínimo 8 Assaltos, 8 Guardas e 4 Ritos. São permitidas até 2 cópias da mesma carta e apenas 1 de cada Lendária.", tip: "Montar 10 / 10 / 10 cria uma base equilibrada; depois você pode personalizar os dez espaços livres." },
...
export function SettingsPage() {
  ...
  return <div className="page settings-page"><header className="page-header"><div><p className="eyebrow">PREFERÊNCIAS LOCAIS</p><h1>Configurações</h1><p>Ajustes ficam somente neste dispositivo.</p></div></header><section className="settings-panel"><h2>Acessibilidade e experiência</h2><label className="setting-select"><span><strong>Ritmo das animações</strong><small>Cinemático deixa voo, comparação e estilhaço mais fáceis de acompanhar.</small></span><select value={preferences.animationPace} onChange={(event) => preferences.setAnimationPace(event.target.value as "cinematic" | "normal" | "quick")}><option value="cinematic">Cinemático · mais legível</option><option value="normal">Normal</option><option value="quick">Rápido</option></select></label>{options.map(([key, title, copy]) => <label className="setting-row" key={key}><span><strong>{title}</strong><small>{copy}</small></span><input type="checkbox" checked={preferences[key]} onChange={(event) => preferences.set(key, event.target.checked)} /><i aria-hidden="true" /></label>)}</section><section className="settings-panel"><h2>Sobre o Alpha</h2><dl className="profile-details"><div><dt>Cliente</dt><dd>Modo Confronto</dd></div><div><dt>Ruleset</dt><dd>{rulesetVersion}</dd></div><div><dt>Privacidade</dt><dd>Tokens na sessão</dd></div><div><dt>Conteúdo</dt><dd>IP original</dd></div></dl></section></div>;
}
