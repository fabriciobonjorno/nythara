...
export function ResultPage() {
  ...
  const shareResult = async () => {
    ...
    try {
      ...
      setShareStatus("O navegador bloqueou a cópia. Tente novamente após permitir acesso à área de transferência.");
    }
  };
  return <div className={`page result-page ${victory ? "victory" : "defeat"}`}>
    <section className="result-hero" aria-labelledby="result-title">
      ...
      <div className="result-duelists" aria-label="Placar final do duelo">
        <article className={`result-duelist ${victory ? "is-winner" : ""}`}>
          <span className="result-duelist__emblem emblem-frame">{myChampion ? <ChampionEmblem id={myChampion.id} faction={myChampion.faction} /> : <UiIcon name="champion" />}</span>
        ...
        <article className={`result-duelist is-rival ${!victory ? "is-winner" : ""}`}>
          <span className="result-duelist__emblem emblem-frame">{rivalChampion ? <ChampionEmblem id={rivalChampion.id} faction={rivalChampion.faction} /> : <UiIcon name="champion" />}</span>
    ...
    <div className="result-content">
      ...
      <aside className="result-command" aria-label="Próximos passos">
        ...
        <button className="primary-button result-rematch" type="button" disabled={practiceBusy} onClick={() => playAgainNow(guided)}><UiIcon name="duel" />{practiceBusy ? "Abrindo a mesa…" : guided ? "Repetir treino guiado" : "Jogar contra o adversário virtual"}</button>
