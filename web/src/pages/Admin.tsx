...
export function AdminPage() {
  ...
  return <div className="page admin-page">
    ...
    <header className="admin-section-title"><div><p className="eyebrow">CONFIGURAÇÃO DO JOGO</p><h2>Operação contínua</h2></div><p>Mudanças auditadas no servidor.</p></header>
    <RulesetsPanel />
...
function OverviewPanel() {
  ...
  return <section className="admin-overview" aria-label="Visão geral operacional">
    ...
    {overview ? <><div className="admin-stat-grid">{stats.map(([label, value, detail]) => <article key={label}>
      <span>{label}</span><strong>{formatNumber(value, locale)}</strong><small>{detail}</small>
...
function PlayersPanel() {
  ...
  return <section className="panel admin-panel admin-players" aria-label="Jogadores">
    ...
    <div className="admin-table-wrap"><table className="admin-table admin-player-table"><thead><tr>
      ...
      <td><strong>{player.display_name}</strong><small>{player.email}<br />desde {formatDate(player.created_at, locale, { dateStyle: "short" })}</small></td>
  ...
  return <section className="panel admin-panel" aria-label="Partidas recentes">
    ...
    <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Partida</th><th>Jogadores</th><th>Modo</th><th>Estado</th><th>Duração</th><th>Início</th></tr></thead>
      <tbody>{matches.map((match) => <tr key={match.id}><td><a href={`/cronica/${match.id}`}><code>{match.id.slice(0, 8)}…</code></a><small>{match.ruleset_version}</small></td>
        ...
        <td>{match.mode === "practice" ? "Treino" : "PvP"}</td><td><span className={`admin-status is-${match.status}`}>{statusLabel(match.status)}</span>{match.end_reason && <small>{endReasonLabel(match.end_reason)}</small>}</td>
        <td>{match.duration_seconds ? formatDuration(match.duration_seconds) : "—"}</td><td>{formatDate(match.created_at, locale, { dateStyle: "short", timeStyle: "short" })}</td></tr>)}</tbody></table></div>
...
function draftStatusLabel(status: string) {
  return ({ draft: "rascunho", validated: "validado", published: "publicado", discarded: "descartado" } as Record<string, string>)[status] ?? "estado desconhecido";
}

function auditActionLabel(action: string) {
  return ({
    "admin_invite:create": "convite administrativo emitido", "ban:create": "carta banida", "ban:lift": "banimento de carta removido",
    "draft:create": "rascunho criado", "draft:simulate": "rascunho simulado", "draft:update": "rascunho atualizado", "draft:validate": "rascunho validado",
    "player:ban": "jogador banido", "player:unban": "jogador liberado", "ruleset:activate": "conjunto de regras ativado",
    "ruleset:publish": "conjunto de regras publicado", "ruleset:rotate": "conjunto de regras rotacionado", "season:create": "temporada criada",
    "season:rewards": "recompensas da temporada distribuídas",
  } as Record<string, string>)[action] ?? "ação administrativa";
}
...
function AdminInvitesPanel() {
  ...
  return <section className="panel admin-panel admin-invites" aria-label="Convites administrativos">
    ...
    {invites.length > 0 && <ul className="admin-invite-list">{invites.map((invite) => { const expired = !invite.used_at && new Date(invite.expires_at).getTime() <= Date.now(); return <li key={invite.id}>
      <div><strong>{invite.email}</strong><small>emitido em {formatDate(invite.created_at, locale, { dateStyle: "short", timeStyle: "short" })} · expira {formatDate(invite.expires_at, locale, { dateStyle: "short", timeStyle: "short" })}</small></div>
...
function RulesetsPanel() {
  ...
  const act = async (kind: "activate" | "rotate", version: string) => {
    const warning = kind === "activate"
      ? `Ativar ${version}? O pareamento passa a usar esta versão imediatamente.`
      : `Rotacionar para ${version}? Concede a coleção da versão a todos e clona baralhos válidos.`;
    if (!window.confirm(warning)) return;
    setBusy(version + kind);
    try {
      if (kind === "activate") {
        await api<void>(`/v1/admin/rulesets/${version}/activate`, { method: "POST" });
        notify("ok", `${version} ativada — pareamento redirecionado.`);
      } else {
        const result = await api<{ granted: number; decks_cloned: number }>(`/v1/admin/rulesets/${version}/rotate`, { method: "POST" });
        notify("ok", `Rotação para ${version}: ${result.granted} concessões, ${result.decks_cloned} baralhos clonados.`);
      }
      reload();
    } catch (caught) { notify("erro", errorText(caught)); }
    finally { setBusy(""); }
  ...
  return <section className="panel admin-panel" aria-label="Conjuntos de regras">
    <header><p className="eyebrow">VERSÕES DE REGRA</p><h2>Conjuntos de regras</h2></header>
    <table className="admin-table"><thead><tr><th>Versão</th><th>Estado</th><th>Publicada</th><th /></tr></thead>
      <tbody>{rulesets.map((ruleset) => <tr key={ruleset.version}>
        ...
        <td>{formatDate(ruleset.created_at, locale, { dateStyle: "short" })}</td>
...
function SeasonPanel() {
  ...
  return <section className="panel admin-panel" aria-label="Temporada">
    ...
    {season ? <p className="admin-season"><strong>{season.name}</strong><small>conjunto de regras {season.ruleset_version} · desde {formatDate(season.starts_at, locale, { dateStyle: "short" })}</small></p>
...
function AuditPanel() {
  ...
  return <section className="panel admin-panel" aria-label="Auditoria">
    ...
    {entries.length ? <ul className="admin-audit">{entries.map((entry) => <li key={entry.id}>
      <code>{auditActionLabel(entry.action)}</code><span>{entry.subject}</span>
      <small>{entry.actor.slice(0, 8)}… · {formatDate(entry.created_at, locale, { dateStyle: "short", timeStyle: "short" })}</small>
...
function DraftsPanel() {
  ...
  const create = async () => {
    try {
      ...
      notify("ok", `Rascunho ${draft.id.slice(0, 8)}… criado para ${draft.card_id}.`);
      setSelected(draft.id); reload();
    } catch (caught) { notify("erro", errorText(caught)); }
  };

  const step = async (action: "validate" | "simulate" | "publish") => {
    if (!selected) { notify("erro", "Selecione um rascunho."); return; }
    ...
    try {
      ...
      notify("ok", `${({ validate: "Validação", simulate: "Simulação", publish: "Publicação" } as const)[action]} concluída.`); reload();
    } catch (caught) { setOutput(""); notify("erro", errorText(caught)); }
  };

  return <section className="panel admin-panel" aria-label="Forja de cartas">
    <header><p className="eyebrow">FORJA DE CARTAS</p><h2>Rascunhos</h2></header>
    <div className="admin-drafts">
      <div className="admin-drafts__editor">
        ...
        <div className="admin-form">
          <input placeholder="Nota do rascunho" value={note} onChange={(event) => setNote(event.target.value)} />
          <button type="button" onClick={create}>Criar rascunho</button>
        </div>
      </div>
      <div className="admin-drafts__list">
        {drafts.length ? <ul className="admin-list">{drafts.map((draft) => <li key={draft.id} className={draft.id === selected ? "is-selected" : ""}>
          <button type="button" className="admin-draft-pick" onClick={() => setSelected(draft.id)}>
            <code>{draft.card_id}</code><small>{draftStatusLabel(draft.status)}{draft.published_version ? ` → ${draft.published_version}` : ""} · {draft.note || "sem nota"}</small>
          </button>
        </li>)}</ul> : <p className="admin-empty">Nenhum rascunho ainda — a forja está fria.</p>}
    ...
    <small>Fluxo: rascunhar → validar (esquema + DSL + compilação) → simular (réplicas sem interface) → publicar versão imutável.</small>
    <Feedback text={feedback} error={isError} />
  </section>;
}
