...
import { useActiveRulesetVersion, useCards, useChampions, useCollection, useDecks, useProgress, useRuleset } from "../queries";
...
export function DeckBuilderPage() {
  ...
  if (cardsQuery.isError || championsQuery.isError || collectionQuery.isError || decksQuery.isError || progressQuery.isError) {
	return <div className="deck-builder-page"><header className="deck-builder-top"><div><p className="eyebrow">SEU ÚNICO BARALHO</p><h1>Montar baralho</h1></div></header><DataLoadError onRetry={() => { void cardsQuery.refetch(); void championsQuery.refetch(); void collectionQuery.refetch(); void decksQuery.refetch(); void progressQuery.refetch(); }} /></div>;
  }
  ...
  return <div className="deck-builder-page confront-builder">
    ...
    <div className="deck-builder-layout">
      <section className="deck-workbench">
        <div className="builder-section avatar-cosmetic"><div className="builder-heading"><div><span>1</span><h2>Escolha o Avatar</h2></div><small>Poder próprio · mesma coleção.</small></div><div className="champion-picker">{avatars.map((avatar) => <button type="button" disabled={locked} className={avatar.id === avatarId ? "is-selected" : ""} onClick={() => setAvatarId(avatar.id)} key={avatar.id}><span className="champion-picker__emblem emblem-frame"><ChampionEmblem id={avatar.id} faction={avatar.faction} /></span><strong>{avatar.name.split(",")[0]}</strong><small>{avatar.confront_power ?? "Sem poder neste conjunto de regras."}</small></button>)}</div></div>

			<div className="builder-section"><div className="builder-heading"><div><span>2</span><h2>Escolha 30 cartas</h2></div><button className="secondary-button" type="button" disabled={locked} onClick={autoBuild}>Montar 10 / 10 / 10</button></div><p className="builder-pool-note"><strong>{legalCards.length} cartas jogáveis</strong> neste modo. Sua conta está no nível {accountLevel}; Lendárias aparecem no catálogo, mas só entram no baralho após o nível indicado. {archivedCount} cartas do conjunto de regras antigo ficam no arquivo.</p><div className="filter-bar compact builder-filters"><input aria-label="Buscar cartas" type="search" placeholder="Buscar por nome, ação ou facção…" value={search} onChange={(event) => setSearch(event.target.value)} /><select aria-label="Filtrar por tipo" value={type} onChange={(event) => setType(event.target.value as typeof type)}><option value="Todos">Todos</option><option value="Assalto">Assalto</option><option value="Guarda">Guarda</option><option value="Rito">Rito</option></select></div><div className="builder-card-grid">{visible.map((card) => <CardTile compact key={card.id} card={card} quantity={maxCopies(card)} selected={counts[card.id] ?? 0} currentLevel={accountLevel} disabled={locked || total >= DECK_SIZE || (counts[card.id] ?? 0) >= maxCopies(card)} onSelect={() => add(card)} />)}</div></div>
      </section>
