...
func main() {
	...
	ruleset := flag.String("ruleset", defaults.RulesetVersion, "versão do ruleset registrada na engine (vazio = competitivo atual)")
	...
	cfg := sim.Config{Games: *games, BaseSeed: *seed, Workers: *workers, MaxCommands: *maxCommands,
		...
		DeckMode: sim.DeckMode(*deckMode), RulesetVersion: *ruleset}
