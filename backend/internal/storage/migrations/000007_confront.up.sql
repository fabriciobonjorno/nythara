ALTER TABLE decks
    ...
    ADD COLUMN system_provided boolean NOT NULL DEFAULT false;

-- O ruleset antigo pode conservar vários decks para replay. No Confronto há
-- exatamente um por conta, protegido também contra corridas entre requests.
CREATE UNIQUE INDEX one_confront_deck_per_user
    ON decks(user_id, ruleset_version)
    WHERE ruleset_version = 'alpha-0.9.0';
