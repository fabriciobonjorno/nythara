-- O modo é propriedade do ruleset, não de um nome de versão específico.
-- Assim versões de balanceamento derivadas de alpha-0.9 continuam usando o
-- fluxo Confronto e a validação de baralho de 30 cartas.
ALTER TABLE rulesets
    ADD COLUMN mode text NOT NULL DEFAULT 'legacy'
        CHECK (mode IN ('legacy', 'confront'));

UPDATE rulesets SET mode='confront' WHERE version='alpha-0.9.0';
...
BEGIN
    ...
    IF NOT FOUND THEN
        RETURN;
    END IF;
    SELECT mode INTO rules_mode FROM rulesets WHERE version=deck_row.ruleset_version;
